...
document.addEventListener('DOMContentLoaded',function(){
    ...
    addProductButton.addEventListener('click',async(e)=>
    {
        ...
        alert(productId+" "+productName);
        try { 
//        roductwithdeemas.kesug.com
//https://productwithdeemas.kesug.com/projects/product/php_rep
        ...
        alert('Failed to Add this Product at this Moment. Please try again.'); 
    }
    ...
    showDetailsButton.addEventListener('click',async (e)=>{
        ...
        if(currentdisplay==='none')
        {
            try {
             ...
             const response = await fetch(`/fetch_product.php?product_id=${product_id}`);
    ...
    updateProductImageButton.addEventListener('click',async()=>
        {
            ...
            try { 
                const response = await fetch('/changeProductImage.php'
                    ,{ 
                        method: 'POST',
                        body:formData
                    }
    ...
    const update_product_availability_btn=document.getElementById('update-product-availability-btn');

    update_product_availability_btn.addEventListener('click',async(e)=>
    {
//        e.preventDefault();
        alert("update product Availability..");

        const product_id=document.getElementById('product-id-for-update').value;
        const product_availability=document.getElementById("yesNoDropdown").value;

        if(product_availability!=='0' && product_availability!=='1')
        {
        ...
        // formData.append('product_availability',product_availability);
        console.log(product_id+" "+product_availability);
        try { 

            
                const response = await fetch('/updateProductAvailable.php'
                    , { 
                        method: 'POST',
                        body: JSON.stringify({ 
            product_id: product_id,
    ...
    deleteProductButton.addEventListener('click',async(e)=>
    {
        ...
        try{
            ...
            const response = await fetch(`/delete_product.php?product_id=${product_id}`);
